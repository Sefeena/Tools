...
class stock_move(models.Model):
    ...
    def check_user_location_rights(self):
        for rec in self:
            if rec.env.user.has_group('warehouse_stock_restrictions.group_restrict_warehouse'):
                if rec.state == 'draft':
                    return True
...
class allow_warehouse(models.Model):
    ...
    warehouse_loc_id = fields.Many2one(
        ...
        check_company=True, default=_get_wh_user_default)
